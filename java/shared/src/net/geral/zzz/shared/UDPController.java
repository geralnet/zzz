...
public class UDPController extends Thread {
	...
	private void fireUdpException(Exception e) {
		for (UDPControllerListener l : listeners
				.getListeners(UDPControllerListener.class)) {
			l.udpException(e);
		}
	}
	...
	public static void send(byte[] bytes) {
		send(new UDPMessage(bytes, true, XController.getConfiguration()
				.getIpString(), XController.getConfiguration().getUdpPort()));
	}
}