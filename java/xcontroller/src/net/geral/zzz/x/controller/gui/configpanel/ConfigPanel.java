package net.geral.zzz.x.controller.gui.configpanel;
...
import net.geral.zzz.x.controller.gui.configpanel.tables.InputTable;
import net.geral.zzz.x.controller.gui.configpanel.tables.OutputTable;

public class ConfigPanel extends JPanel {
	private static final long	serialVersionUID	= 1L;
	private final JTextField	txtMAC;
	private final JTextField	txtIP;
	private final JTextField	txtMask;
	...
	private final JTextField	txtUdpPort;
	private final InputTable	tableInputs;
	private final OutputTable	tableOutputs;
	private final JCheckBox		chkDHCP;
	...
	public ConfigPanel() {
		...
		panelIdentification.setBorder(new TitledBorder(null, " zZz Identification", TitledBorder.LEADING,
		...
		panelNetwork.setBorder(new TitledBorder(null, "Network Settings", TitledBorder.LEADING, TitledBorder.TOP, null,
				null));
		...
		panelInputs.setBorder(new TitledBorder(null, "Inputs Configuration", TitledBorder.LEADING, TitledBorder.TOP,
				null, null));
		...
		panelOutputs.setBorder(new TitledBorder(null, "Outputs Configuration", TitledBorder.LEADING, TitledBorder.TOP,
				null, null));
	...
	public ZConfiguration getConfiguration() {
		...
		try {
			...
			IPv4 ip = new IPv4(txtIP.getText());
			cfg.setIp(ip.getBytes());

			ArrayList<ZOutputConfiguration> data = tableOutputs.getModel().getAll();
			cfg.setOutputConfiguration(data.toArray(new ZOutputConfiguration[data.size()]));
			return cfg;
		}
		catch (Exception e) {
			// TODO handle it
