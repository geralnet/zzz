...
import net.geral.zzz.x.controller.comm.UDPController;

public class TrayMenu extends TrayIcon implements ActionListener, MouseListener, ItemListener {
	private final Menu	optionsMenu	= createOptionsMenu();
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if ("CONFIG".equals(cmd)) config();
		...
		else System.err.println("Invalid command: " + cmd);
	}
	...
	private void createInputsMenu(Menu menu) {
		...
		for (ZOutputConfiguration o : os) {
			if (!groups.contains(o.getGroup())) groups.add(o.getGroup());
		}
		...
		for (String g : groups) {
			...
			for (ZOutputConfiguration o : os) {
				if (g.equals(o.getGroup())) {
					...
					CheckboxMenuItem item = new CheckboxMenuItem(o.getName(), state);
	...
	public void itemStateChanged(ItemEvent e) {
		...
		ZMessageType turnOnOrOff = cb.getState() ? ZMessageType.REQUEST_TURNON_ONE : ZMessageType.REQUEST_TURNOFF_ONE;
		byte[] bytes = new byte[] { (byte) turnOnOrOff.getBytecode(), (byte) nInput };
	...
	public void mouseClicked(MouseEvent e) {
		if (e.isPopupTrigger()) createPopUp();
	}
